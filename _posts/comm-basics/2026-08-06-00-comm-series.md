...
| EtherCAT | 18 | 다축 로봇 제어의 표준 | 공개 |
| 종합 | 5 | 비교표, 선택 기준, 반복 패턴 | 공개 |
...
조인트 안에서 가장 안쪽이다. 엔코더와 ADC 와 IMU 와 게이트 드라이버가 여기 붙는다. 이 폴더의 결론은 오류 검출이 사실상 없다는 것이고, 그 전제가 언제 깨지는지를 다룬다.
...
## EtherCAT — 프레임을 하나만 쓴다 (18편)

처음 보는 사람은 [00. 한눈에 보는 EtherCAT](/posts/00-ethercat-overview/) 부터 읽으면 된다. 이 부 전체의 지도다.

앞의 넷이 못 푼 칸을 푼다. 발상 하나가 전부다. 프레임이 하나면 스케줄할 게 없다. 나머지 열여섯 편은 그 하나를 성립시키기 위해 생긴 장치들이고, 각 편을 읽을 때 "이게 on-the-fly 를 위해 왜 필요한가" 를 물으면 전부 연결된다.

| # | 글 |
| --- | --- |
| 00 | [**한눈에 보는 EtherCAT: 처음 읽는 사람을 위한 지도**](/posts/00-ethercat-overview/) |
| 01 | [왜 이더넷 위에 필드버스를 얹었나](/posts/01-why-fieldbus-on-ethernet/) |
| 02 | [on-the-fly 처리, 이 프로토콜의 전부](/posts/02-on-the-fly-processing/) |
| 03 | [토폴로지와 물리계층, MII와 EBUS](/posts/03-topology-mii-ebus/) |
| 04 | [프레임 구조, Datagram과 WKC](/posts/04-frame-datagram-wkc/) |
| 05 | [주소 지정, 자동증가와 설정과 논리](/posts/05-addressing-auto-fixed-logical/) |
| 06 | [FMMU, 논리 주소를 접는다](/posts/06-fmmu-logical-mapping/) |
| 07 | [SyncManager, 버퍼 모드와 메일박스 모드](/posts/07-syncmanager-buffer-mailbox/) |
| 08 | [ESM, INIT과 PREOP과 SAFEOP과 OP](/posts/08-esm-state-machine/) |
| 09 | [분산 클록(DC)](/posts/09-distributed-clocks/) |
| 10 | [프로세스 데이터와 PDO 매핑](/posts/10-process-data-pdo-mapping/) |
| 11 | [메일박스 프로토콜, CoE와 FoE와 EoE](/posts/11-mailbox-coe-foe-eoe/) |
| 12 | [CoE와 CiA 402, 드라이브 기동 절차](/posts/12-coe-cia402-bringup/) |
| 13 | [ESI와 ENI 설정 파일](/posts/13-esi-eni-config-files/) |
| 14 | [마스터 구현, SOEM 코드로 읽기](/posts/14-soem-master-implementation/) |
| 15 | [사이클 지터와 실시간 OS](/posts/15-cycle-jitter-realtime-os/) |
| 16 | [진단, WKC와 Lost Frame과 CRC 카운터](/posts/16-diagnostics-wkc-crc-counters/) |
| 17 | [예제, 마스터 골격 만들기](/posts/17-ethercat-master-skeleton/) |

## 종합 — 프로토콜을 나란히 놓고 비교 (5편)

지금까지는 프로토콜을 하나씩 따라갔다. 이 폴더는 순서를 바꿔 같은 문제를 프로토콜별로 모은다. 같은 문제에 대한 서로 다른 답을 나란히 놓으면 여덟 개의 반복 패턴이 드러나고, 그게 프로토콜 문법보다 오래 쓰인다.

| # | 글 |
| --- | --- |
| 01 | [한 장 비교표](/posts/01-protocol-comparison-table/) |
| 02 | [어느 것을 언제 고르나](/posts/02-protocol-selection-guide/) |
| 03 | [계층 대응표, 같은 개념 다른 이름](/posts/03-layer-mapping-table/) |
| 04 | [공통 실패 패턴 10가지](/posts/04-common-failure-patterns/) |
| 05 | [통합 디버깅 체크리스트](/posts/05-integrated-debug-checklist/) |
...
각 폴더의 마지막 편이 그 통신의 예제다. C++17 로 쓰고 ROS 2 패키지 구조를 따른다. 원칙은 **프로토콜 로직과 I/O 를 분리**하는 것이다. 하드웨어를 인터페이스와 fake 구현으로 나누면 실물 없이 CI 에서 검증된다.
