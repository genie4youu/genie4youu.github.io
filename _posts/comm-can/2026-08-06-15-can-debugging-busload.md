...
간헐적 문제는 `candump -l` 로 기록해두는 습관으로 잡는다. 가끔 이상하다를 잡으려면 이상할 때의 기록이 있어야 한다. 운영 중에 항상 실행해두고 순환 저장하는 것도 방법이다.
...
- 오류 카운터를 본다. 리눅스는 `berr-reporting on` 을 켜야 종류별 정보가 온다.
...
- Bus Off 진단에서는 TEC 가 서서히 올랐는지 갑자기 뛰었는지를 본다. TEC 히스토리를 링 버퍼로 남긴다.
