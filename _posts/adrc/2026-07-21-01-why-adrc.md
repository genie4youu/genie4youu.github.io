...
ADRC를 "PID보다 나은 것"으로만 보면 구조를 놓친다. Han의 논문 제목이 *From PID to ADRC*인 이유는, ADRC가 PID를 부정한 것이 아니라 PID의 장점을 지키려고 나왔기 때문이다.
...
이 넷은 튜닝 실패가 아니라 **구조에서 오는 한계**다. 게인을 아무리 잘 잡아도 계단 입력은 여전히 계단이고, 미분은 여전히 노이즈를 키운다.
