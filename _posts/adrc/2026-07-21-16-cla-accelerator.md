...
빠르고 결정적이어야 하는 안쪽 루프(ADRC)는 CLA, 유연하지만 지터가 있어도 되는 바깥일(통신과 감독)은 CPU다. 이 분업이 15편의 "연속 제어 + 상위 FSM 감독"을 하드웨어 레벨로 나눈 모습이다.

## ⚠️ 주의

- CLA는 작은 코어다. 명령셋과 메모리가 제한적이라 분기 많은 로직은 부적합하고 수치 제어 루프에 특화돼 있다.
