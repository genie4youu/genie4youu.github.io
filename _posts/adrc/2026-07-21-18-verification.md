...
이론은 직접 돌려보지 않으면 안다고 할 수 없다. 두 경로로 확인한다. 하나는 제품이 제공하는 Simulink ADRC 블록으로 빠르게 거동을 보고 PID와 비교하는 것이고, 다른 하나는 ESO와 제어법칙을 직접 코드로 옮겨 원리를 검증하는 것이다. 아래 예제 코드는 공개 원리로 처음부터 작성한 것이다.
...
| 외란 주입 | 외란 억제 | ESO가 추정하고 상쇄해 회복이 빠름 |
| 파라미터 변화 | 강인성 | PID는 재튜닝 필요, ADRC는 버팀 |
| 측정 노이즈 | 노이즈 민감도 | $$\omega_o$$ 크면 입력 떨림 확인 |

같은 플랜트에 PID와 ADRC를 나란히 둔다. 외란 주입과 파라미터 변화에서 차이가 극적으로 드러난다. 계단만 보면 정상 상태에서는 등가라 차이가 안 보인다.
...
```mermaid
flowchart LR
    A["총외란과 ESO"] --> B["대역폭 파라미터화"] --> C["차수와 b0"] --> D["이산화와 구현"] --> E["조인트 적용"]
```
