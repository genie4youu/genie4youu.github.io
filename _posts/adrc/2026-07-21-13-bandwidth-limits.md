...
지연은 위상여유를 깎는다. 위상 지연은 $$\omega \cdot t_{delay}$$이므로 대역폭이 클수록 같은 지연이 더 큰 위상 손실이 된다. 연산과 통신 주기가 대역폭 상한을 직접 정한다.
...
조인트는 감속기와 링크 탄성 공진이 있다. 대역폭을 그 아래로 유지해야 한다. 노치 필터를 쓰거나 대역폭을 최저 공진의 3분의 1에서 5분의 1 아래로 둔다.
