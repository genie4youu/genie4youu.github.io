...
프레이밍 파서는 본질적으로 FSM 이다. 그리고 바이트가 몇 개씩 쪼개져 도착하므로(UART 인터럽트, 소켓 read) 중간에 멈췄다가 이어갈 수 있어야 한다. 상태를 명시적으로 두는 게 정답이다.
...
| 지점 | 왜 중요한가 |
| --- | --- |
| `WaitStx` 에서 아무 바이트나 버림 | 재동기의 핵심이다. 어긋나도 다음 STX 에서 복구된다 |
