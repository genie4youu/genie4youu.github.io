...
이 코드에서 볼 것이 셋이다. 블로킹하지 않고 매 주기 호출되며 상태를 보고 다음 명령을 낸다. `while` 루프로 짜면 제어 주기가 깨진다. 상태 타임아웃을 둬서 어느 상태에서 안 나가면 문제가 있다는 걸 알린다. 폴트 리셋 횟수를 제한한다. 무한 리셋은 위험하고 [08편](/posts/08-can-error-states-busoff/)의 Bus Off 백오프 정책과 같은 발상이다.
