...
description: 어느 프로토콜이 좋은가로 시작하면 안 된다. 그리고 실무에서 판단을 뒤집는 1순위는 부품이 그것만 지원한다는 사실이다.
categories: [로봇 통신, 종합]
...
"모터를 구동한 상태에서" 라는 조건을 붙였다. 정지 상태에서 잘 되던 통신이 모터를 구동하면 깨지는 게 [기초 11편](/posts/11-basics-noise-ground-isolation/)의 주제였다. 온도도 마찬가지다. [시리얼 02편](/posts/02-uart-baud-error-budget/)에서 본 발진자 드리프트와 커넥터 열팽창은 시간이 지나야 나타난다.
