...
**`before` 여야 한다.** 실행 후 보고가 아니라 실행 전에 인자까지 제시하고 확인을 받아야 한다.
...
사용자에게 링크 전달
쿠키가 있으므로 동의 화면이 건너뛰어진다   ← 여기
인가 코드가 공격자 서버로 리다이렉트
```

**"1회 동의의 흔적이 이후 동의를 자동 통과시킨다"는 구조다.** 사용자는 승인하지 않았는데 시스템은 승인된 것으로 취급한다.
...
- confused deputy는 **1회 동의 흔적이 이후 동의를 건너뛰게 하는 것**이다
