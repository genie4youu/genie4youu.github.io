...
규칙은 두 가지다. 침묵은 새 프레임이 시작됐다는 힌트로만 쓰고 필수 조건으로 삼지 않는다. 그리고 길이를 알면 침묵을 기다리지 않고 즉시 CRC 로 확정한다. 응답 지연이 t3.5 만큼 줄어든다. 이러면 latency timer 가 16 ms 여도 바이트가 한 덩어리로 오든 쪼개져 오든 똑같이 동작한다.
