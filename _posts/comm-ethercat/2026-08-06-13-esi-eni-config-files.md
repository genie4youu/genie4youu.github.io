...
| 내용 | 그 장비 종류의 전체 명세 | 그 명세의 핵심 부분 | 네트워크 전체 구성 + 마스터의 사이클 명령 |
| CANopen 대응 | EDS | 없음 | DCF에 가깝다 |

핵심 차이는 이렇다. ESI와 SII는 이 장비가 무엇을 할 수 있는지(능력)를 말하고, ENI는 이 시스템을 어떻게 돌릴 것인지(설정)를 말한다.
...
ESI의 핵심 부분이 슬레이브의 EEPROM에 기록되어 있다.
...
사이클 명령 목록이 ENI의 핵심이다. 마스터가 런타임에 계산하지 않고 미리 만들어진 명령을 그대로 실행한다. 결정적이고 빠르다.
...
ESI를 소스 저장소에 함께 넣는다. "그때 어떤 ESI로 만들었나" 가 재현성의 핵심이다. 제조사 웹사이트에서 다시 받으면 버전이 달라져 있을 수 있다.
...
- 세 가지를 구분한다. ESI(제조사 XML, 장비 명세), SII/EEPROM(슬레이브 안의 핵심 정보), ENI(설정 도구가 만든 네트워크 구성)
...
- ENI의 핵심은 사이클 명령 목록이다. 마스터가 런타임 계산 없이 미리 만들어진 명령을 실행한다
