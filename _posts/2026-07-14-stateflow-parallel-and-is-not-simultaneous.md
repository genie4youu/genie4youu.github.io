...
> — 병렬(AND) 상태는 동시처럼 보이지만, 시뮬레이션 중에는 **순차적으로 실행된다.**
{: .prompt-info }
...
각 병렬 상태의 **오른쪽 위에 숫자**가 붙는다. 이게 실행 순서다.

```mermaid
flowchart TB
    subgraph Chart["Chart (PARALLEL_AND 분해)"]
        direction TB
...
> 여기서 중요한 건 **기본값이 "내가 상태를 그린 순서"** 라는 점이다.
...
> 각 상태는 자기가 실행될 때만 데이터를 읽고 쓴다. 그 결과, 어떤 상태는 **다른 상태가 이미 써 놓은 데이터를 덮어쓸 수 있다.**
{: .prompt-info }
...
[MAB 모델링 가이드라인 `jc_0722`](https://www.mathworks.com/help/simulink/mdl_gd/maab/jc_0722localdatadefinitioninparallelstates.html) 는 아예 **데이터의 소유자를 하나로 두라**고 한다.
...
두 번째를 순서 조정으로 때우려 하면, 겉보기엔 동작하지만 **상태가 하나 추가되는 순간 무너진다.** 번호가 밀리기 때문이다.

---

## 5. 차트가 깨어날 때 정확히 무슨 일이 일어나나

병렬 실행 순서를 이해했으면, 한 단계 더 들어가 **상태 하나가 실행될 때의 순서**도 알아두면 좋다.

```mermaid
flowchart TD
    W(["차트가 깨어난다<br/>(wake up)"]) --> O{"① 나가는 outer<br/>transition 이 유효한가?"}
    O -->|"유효"| T["천이 실행<br/>→ 끝"]
    O -->|"없음"| D["② during 액션 실행"]
    D --> I{"③ inner transition<br/>평가"}
    I --> C["④ 활성 자식 상태로 내려가<br/>①부터 반복"]
    C --> S(["잠든다<br/>(go to sleep)"])
...
> **`during` 은 백그라운드에서 상시 도는 코드가 아니다.**
>
> 유효한 outer transition이 **없을 때만** 그 스텝에 한 번 실행된다.
> **천이가 일어나면 `during` 은 실행조차 되지 않는다.**
{: .prompt-danger }

"상태에 머무는 동안 계속 도는 코드"라고 생각하면 틀린다. 위 그림에서 `during` 이 **분기 아래**에 있다는 게 전부다.
...
- [ ] 애초에 병렬 상태끼리 **변수를 공유하지 않는 게 최선**이다 (MAB `jc_0722`)
- [ ] 공유가 불가피하면, **실행 순서가 곧 설계 사양이다.** 문서에 남겨라 — 나중에 누가 상태 하나를 추가하면 번호가 밀린다
- [ ] `during` 에 **"항상 실행돼야 하는 것"** 을 넣지 않았는가?

### 한 줄로

> **병렬 상태는 "동시에 활성"이지 "동시에 실행"이 아니다.**
> 공유 변수가 있는 순간, **실행 순서는 그림이 아니라 사양이 된다.**
{: .prompt-tip }
...
`{조건 동작}` 과 `/천이 동작` 의 차이, 그리고 **백트래킹**이 만드는 함정.
