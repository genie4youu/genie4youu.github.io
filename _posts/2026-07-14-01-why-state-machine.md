...
description: 상태가 늘어날 때 if 문이 무너지는 지점. 상태 기계가 무엇을 해결하는지 배터리 예제로 확인한다.
...
발상은 단순하다. 모드를 변수 조합에서 추론하지 말고, 그냥 이름을 붙여서 명시하자는 것이다.
...
`if` 문의 한계는 분기 개수가 아니라 모드가 코드에 표현되지 않는다는 데 있다. 상태 기계는 모드에 이름을 붙여 명시하고, State와 Transition 두 가지만으로 그걸 해낸다. Stateflow는 그것을 그리고, 시뮬레이션하고, 검사하게 해준다.

---

> **1부 Stateflow 시작하기 (1/7)** — [전체 목록](/learning-map/)
>
